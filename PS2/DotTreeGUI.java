...
public class DotTreeGUI extends DrawingGUI {
	...
	public void handleMousePress(int x, int y) {
		if (mode == 'a') {
			...
			// TODO: YOUR CODE HERE
			// if tree is null, create a new quadtree with Dot at x and y
			if (tree == null) {
				tree = new PointQuadtree<>(new Dot(x, y), 0, 0, width, height);
			}
			// else insert new Dot at x and y
			else {
			...
			// TODO: YOUR CODE HERE
			// set found equal to the list of points in circle using method from PointQuadtree.java
			found = tree.findInCircle(x, y, mouseRadius);
			// print the size of found
			System.out.println("the size of found is " + found.size());
		}
		else {
	...
	/**
	 * test tree 2 -- we made this one
	 * hardcoded point locations for 800x600
	 */
	private void test2() {
		found = null;
		tree = new PointQuadtree<Dot>(new Dot(200,200), 0,0,800,600); // start with A
		tree.insert(new Dot(99,100)); // B
		tree.insert(new Dot(320, 300)); // C
		tree.insert(new Dot(111,390)); // D
		tree.insert(new Dot(300,118)); // E
		tree.insert(new Dot(690,40)); // F
		int bad = 0;
		bad += testFind(200,200,10,6,5,1); 	// rect for all; circle for all except one; find A
		bad += testFind(40,60,20,5,2,0);	// rect for all except one; circle for two; find G
		bad += testFind(100,250,180,6,5,3);	// rect for all; circle for all except one; find B,D,A
		bad += testFind(790,248,69,6,3,0);	// rect for all; circle for three; find none
		bad += testFind(400,333,80,5,2,0);	// rect for all except one; circle for two; find none
	...
	public void drawTree(Graphics g, PointQuadtree<Dot> tree, int level) {
		...
		// set x and y equal to tree's point's x and y values
		int x = (int)tree.getPoint().getX();
		int y = (int)tree.getPoint().getY();

		// draw dot
		g.fillOval(x - dotRadius, y - dotRadius, 2*dotRadius, 2*dotRadius);

		// draw lines for each tree
		g.drawLine(x, tree.getY1(), x, tree.getY2()); // draw vertical line
		g.drawLine(tree.getX1(), y, tree.getX2(), y); // draw horizontal line

		// Recurse with children
		// TODO: YOUR CODE HERE
		// for every child, recurse if that child exists
		for (int i=1; i<5; i++) {
			if (tree.hasChild(i)) {
