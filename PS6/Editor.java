...
public class Editor extends JFrame {
	...
	public Editor() {
		...
		// Put the buttons anwd canvas together into the window
	...
	public void drawSketch(Graphics g) {
		// TODO: YOUR CODE HERE
        for (Shape shape : sketch.getIDToShape().values()) {
            shape.draw(g);
        }
		if (curr != null) {
			curr.draw(g);
	...
	private void handlePress(Point p) {
        ...
        if (mode == Mode.DRAW) {
        	...
        	movingId = sketch.onShape((int)p.getX(), (int)p.getY());
			if (movingId != -1) {
				moveFrom = new Point((int)p.getX(), (int)p.getY());
			}
        }
        else if (mode == Mode.RECOLOR) {
			int shapeId = sketch.onShape((int)p.getX(), (int)p.getY());
			if (shapeId != -1) {
	...
	private void handleDrag(Point p) {
		...
		if (mode == Mode.DRAW) {
			if (shapeType.equals("ellipse")) {
				...
				((Polyline) curr).getPointPairs().add((int)p.getX());
				((Polyline) curr).getPointPairs().add((int)p.getY());
			}
			else if (shapeType.equals("segment")) {
			...
			if (movingId != -1) {
				comm.send("move " + movingId + " " + (int)moveFrom.getX() + " " + (int)moveFrom.getY() + " " + (int)p.getX() + " " + (int)p.getY());
				moveFrom = p;
			}
		}
		repaint();
